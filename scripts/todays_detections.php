...
$totalcount = $dataLayer->getTotalCount()->fetchArray(SQLITE3_ASSOC);
$todaycount = $dataLayer->getTodayCount()->fetchArray(SQLITE3_ASSOC);
$hourcount = $dataLayer->getHourCount()->fetchArray(SQLITE3_ASSOC);
        ...
        <tr class="relative" id="<?php echo $iterations; ?>">
          <td class="relative"><a target="_blank" href="index.php?filename=<?php echo $todaytable['File_Name']; ?>"><img
            ...
            <video onplay='setLiveStreamVolume(0)' onended='setLiveStreamVolume(1)' onpause='setLiveStreamVolume(1)' controls
              poster="<?php echo $filename . ".png"; ?>" preload="none" title="<?php echo $filename; ?>">
  ...
  <table class="ci-todays-detections">
    ...
    <tr>
      ...
      <form action="" method="GET">
        <td><input type="hidden" name="view" value="Recordings"><button type="submit" name="date"
            value="<?php echo date('Y-m-d'); ?>">
            <?php echo $todaycount['COUNT(*)']; ?>
          </button></td>
      </form>
      <td>
        <?php echo $hourcount['COUNT(*)']; ?>
      </td>
      <form action="" method="GET">
        <td><button type="submit" name="view" value="Species Stats">
      ...
      <form action="" method="GET">
        <td><input type="hidden" name="view" value="Recordings"><button type="submit" name="date"
            value="<?php echo date('Y-m-d'); ?>">
            <?php echo $todayspeciestally['COUNT(DISTINCT(Com_Name))']; ?>
          </button></td>
      </form>
    </tr>
  </table>

  <h3>Today's Detections — <input autocomplete="off" size="11" type="text" placeholder="Search..." id="searchterm"
      name="searchterm"></h3>
